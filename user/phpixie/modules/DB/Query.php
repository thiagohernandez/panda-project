 ...
 *
 * @method mixed data(array $data = null) Set data for update queries.
...
abstract class Query
{
	...
	/**
	 * Set a single colume of data in an 'update' query
	 */
	public function set($column, $value) {
		if ($this->_type != 'update') {
			throw new \Exception("set() can only be used with 'update'");
		}
		if (!$this->_data) {
			$this->_data = array();
		}
	...
	private function get_condition_part($p) {
		...
		// Modified by FTB to allow 'null' as a value
		if (is_string($p[0]) || ($p[0] instanceof \PHPixie\DB\Expression)) {
