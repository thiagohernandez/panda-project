...
/**
 * Allows iterating over ORM objects inside loops like 'foreach',
...
class Result implements \Iterator
{
	...
	public function __construct($pixie, $model, $dbresult, $with = array())
	{
		...
		//if (!empty($with)) {$pixie->debug->pretty($with);die;}
		$this->pixie = $pixie;
		$this->_model = $model;
		...
		foreach ($with as $path => $rel)
		{
			...
			$this->_with[] = array(
				...
				'path_count' => count($p),
				'model' => $rel['model'],
				'columns' => $rel['model']->column_names(),
			);
		}
	}
	...
	public function build_model($data) {
		...
		// Load related models when there's a "with" clause in the query
		if (empty($this->_with)) {
			$model->values($data, true, false);
		}
		else {
			// The model's data
			$model_data = array();
			...
			$model->values($model_data, true, false);

			// The related objects (requested by "with")
			foreach ($this->_with as $rel) {
				...
				$rel_model->values($rel_data, true, false);

				$owner = $model;
				foreach ($rel['path'] as $key => $child) {
					if ($key == $rel['path_count'] - 1) {
						$owner->related_objects[$child] = $rel_model;
					}
					else {
						$owner = $owner->related_objects[$child];
					}
				}
			}
		}
