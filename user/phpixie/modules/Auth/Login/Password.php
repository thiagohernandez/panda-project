...
class Password extends Provider {
    ...
    if ($user instanceof \PHPixie\Auth\Login\Password\User) {
      $challenge = $user->password_hash();
		...
		else if ($user instanceof \PHPixie\ORM\Model && $user->loaded()) {
			$password_field = $this->password_field;
			$challenge = $user->$password_field;
			if (empty($challenge) || (strlen($challenge)<8)) {
				return false;
			}
    }
		else {
		...
		if ($this->hash_method && ('panda'==$this->hash_method)) {
			$password = sha1('DYhGoPe9DfkE30fs2guVoUubWwvniRVC7D9D20fG'.$password);
		}
		else if ($this->hash_method && ('crypt'==$this->hash_method)) {
	...
	public function hash_password($password){
		...
		if ('panda'==$this->hash_method) {
			return sha1('DYhGoPe9DfkE30fs2guVoUubWwvniRVC7D9D20fG'.$password);
		}
		else if('crypt'==$this->hash_method) {
