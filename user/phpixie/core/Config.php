 ...
 * You can add configuration files to the /config folder
...
class Config
{
	...
	public function get_group($name)
	{
		if (!isset($this->groups[$name])) {
			$file = $this->make_file_name('config/',$name);
			if (!file_exists($file)) {
				$file = $this->make_file_name('config/app/',$name);
				if (!file_exists($file)) {
					$file = $this->make_file_name('phpixie/files/config/',$name);
					if (!file_exists($file)) {
						$file = false;
					}
				}
	...
	public function write($group) {
		...
		if (isset($this->groups[$group_name])) {
			...
			$v = var_export($group['options'], true);
			$d = "<?php\r\nreturn $v;";
			$result = file_put_contents($group['file'], $d);
			if (($result===false) || ($result != strlen($d))) {
				throw new \Exception("Error writing config file ".$group['file']);
			}
		}
	}
}
